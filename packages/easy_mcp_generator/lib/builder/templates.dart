...
/// response when the server has `@Mcp(codeMode: true)`.
///
/// Only tools that explicitly set `@Tool(codeModeVisible: true)` are kept.
/// When `@Mcp.codeMode` is false this helper is not used — all tools are
...
/// the server has `@Mcp(codeMode: true)`.
///
...
$paramConversions
      ...
      if (_logErrors) {
        io.stderr.writeln('[easy_mcp] $name: \$e');
        io.stderr.writeln(st);
        await io.stderr.flush();
      }
      return CallToolResult(
