...
### Fixed
- Fixed linter warnings in generated MCP server files:
  - Made `easy_api_annotations` import conditional — only emitted when prompts are present
  - Made `dart:math` import conditional — only emitted when code mode is enabled
  - Fixed unnecessary null checks and null-aware operators for `process` and `tempDir` variables in code mode sandbox shutdown logic
  - Generated files now pass `dart analyze` with zero warnings
- Fixed HTTP transport template to always include `dart:io` import (required for `io.Platform.environment['PORT']` and `io.stderr` logging)
